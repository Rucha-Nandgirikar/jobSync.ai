...
# -*- coding: utf-8 -*-
"""
Crawl all Ashby companies from ashby_startups.json
"""
...
async def crawl_all_ashby(companies: list) -> list:
    """Crawl all Ashby companies"""
    all_jobs = []
    
    total = len(companies)
    logger.info(f"Starting to crawl {total} Ashby companies...\n")
...
async def main():
    """Main function"""
    # Load Ashby companies from JSON
    json_path = os.path.join(os.path.dirname(__file__), "../../data/ashby_startups.json")
    
    logger.info("="*80)
    logger.info("ASHBY COMPANY CRAWLER")
    logger.info("="*80)
    ...
    logger.info(f"Found {len(companies_json)} Ashby companies\n")
    logger.info("="*80 + "\n")
    
    # Crawl all
    all_results = await crawl_all_ashby(companies_json)
    ...
    # Save results
    output_path = os.path.join(os.path.dirname(__file__), "../../data/ashby_crawled_jobs.json")
